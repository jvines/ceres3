...
def get_name(t,g,z):
    ...
    return f'{T}_{G}_{siZ}{Z}p00.ms.fits'
...
def CCF(spec, model_path='/dummy/path/',doplot = False, plot_dir = '/home/rabrahm/',plot_name = 'MY_LUP'):
    ...
    while itera < 4:
        ...
        if late==False:
            ...
            #plt.show()
            #print(f'radial velocity = {velo2} km/s')
            #print(f'Sigma = {sig2} km/s')
            """
        ...
        if len(RI) > 0:
            ...
            while i < len(modZ):
                ...
                while ig < len(modG):
                    it = 0
                    while it < len(modT):
                        MK = f'R_{rot}_{get_name(modT[it],modG[ig],modZ[i])}'
            ...
            for i in modZ:
                for ig in modG:
                    for it in modT:
                        MK = f'R_{rot}_{get_name(it,ig,i)}'
                        if os.access(model_path1+MK,os.F_OK):
                            MOD.append(MK)
    ...
    #print(f'Pars fase 0: {Tfin} {Gfin} {Zfin} {rotfin} {maximo}')
    mejor = False
    if rotfin == 0.0:
        nrot = [0.0,2.5]
    elif rotfin == 2.5:
    ...
    if total_inter == 0:
        ...
        #print(f'interp {intert} {interg} {interz} {interv}')
        ...
        for t in nT:
            for g in nG:
                nam = f'R_{rotfin}_{get_name(t,g,Zfin)}'
                I = numpy.where(calc == nam)[0]
    ...
    if doplot:
        ...
        for i in range(L1.shape[0]):
            ...
            plt.ylim(0.,1.01)
            plt.plot(L1[i],F1[i])
            plt.plot(PWAV[I],sc[I])
            plt.xlabel('wavelenth [A]')
            plt.ylabel('Continuum Normalized Flux')
            plt.savefig(f'{plot_dir}{plot_name}_{int(L1[i,0])}_{int(L1[i,-1])}.pdf', format='pdf')
